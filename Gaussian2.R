...
ROC.CD.norm <- function(parametros, marker, time){
  surv.times <- time
  ...
  xi.2    <- parametros[9]
  omega.2 <- exp(parametros[10])
  alpha.2 <- parametros[11]
  norm.cop <- normalCopula(rho.par)
  f.M.sn <- w*dsn(marker, xi=xi.1, omega=omega.1, alpha=alpha.1)+
    (1-w)*dsn(marker, xi=xi.2, omega=omega.2, alpha=alpha.2)
  F.M.sn <- w*psn(marker, xi=xi.1, omega=omega.1, alpha=alpha.1)+
    (1-w)*psn(marker, xi=xi.2, omega=omega.2, alpha=alpha.2)
  f.T.wei <- dweibull(surv.times, shape = shape.par, scale = scale.par)
  F.T.wei <- pweibull(surv.times, shape = shape.par, scale = scale.par)
  g.1.joint.MT <- p*f.M.sn*f.T.wei*dCopula(cbind(F.M.sn, 1-F.T.wei), norm.cop)
  G.1.joint.MT <- pCopula(cbind(F.M.sn, 1-F.T.wei), norm.cop)
  TP.C <- (G.1.joint.MT-F.M.sn-(1-F.T.wei)+1)/F.T.wei
  G.m.t <- (1-p)*F.M.sn + p*G.1.joint.MT
  FP.D <-  1-(G.m.t/((1-p)+p*(1-F.T.wei)))
  distances.from.0.1 <- sqrt((FP.D)^2+(1-TP.C)^2)
  which.minimum <- (1:length(distances.from.0.1))[distances.from.0.1== 
                                                    min(distances.from.0.1)]
  auc.C <- auc(c(1,FP.D,0), c(1,TP.C,0))
  pauc.C <- AUC(c(1,FP.D,0), c(1,TP.C,0), from=0.2, to=0.4)
  cat(c(FP.D[which.minimum], TP.C[which.minimum]), "\n")
  list(AUC.CD=auc.C,
       PAUC.CD=pauc.C,
       CUTOFF.CD=marker[which.minimum], 
...
ROC.ID.norm <- function(parametros, marker, time){
  surv.times <- time
  rho.par   <- tanh(parametros[1])
  p <- pnorm(parametros[2])
  scale.par <- exp(parametros[3])
  ...
  g.1.joint.MT <- p*f.M.sn*f.T.wei*dCopula(cbind(F.M.sn, 1-F.T.wei), norm.cop)
  G.1.joint.MT <- pCopula(cbind(F.M.sn, 1-F.T.wei), norm.cop)
  TP.I <- pnorm((rho.par*qnorm(1-F.T.wei)-qnorm(F.M.sn))/sqrt(1-rho.par^2))
  G.m.t <- (1-p)*F.M.sn + p*G.1.joint.MT
  FP.D <-  1-(G.m.t/((1-p)+p*(1-F.T.wei)))
  distances.from.0.1 <- sqrt((FP.D)^2+(1-TP.I)^2)
  which.minimum <- (1:length(distances.from.0.1))[distances.from.0.1== 
                                                    min(distances.from.0.1)]
  auc.I <- auc(c(1,FP.D,0), c(1,TP.I,0))
  cat(c(FP.D[which.minimum], TP.I[which.minimum]), "\n")
  list(AUC.ID=auc.I, 
       pauc.ID = AUC(c(1,FP.D,0), c(1,TP.I,0), from=0.2, to=0.4),
...
tau.studied <- 0.5
rho.par <- iTau(normalCopula(), tau.studied)

t.10 <- qweibull(0.05, shape=1.5, scale=1)

true.ROC.CD.norm <- ROC.CD.norm(parametros=c(atanh(rho.par),-0.5, 
                                             0, log(1.5),
                                             0,-7,0.7,0.5,1,1,1.5),
                                marker=seq(from=-16, to=11, length=500), time=t.10)

cat("True cut'off marker:", true.ROC.CD.norm$CUTOFF.CD, "\n")
cat("AUC^(CD) true:", true.ROC.CD.norm$AUC, "\n")
plot(true.ROC.CD.norm$FP.D.CD, true.ROC.CD.norm$TP.C.CD, type="l", ylim=c(0,1), xlim=c(0,1),
     xlab="FP", ylab="TP(C)")
points(true.ROC.CD.norm$FP.D.at.cutoff, true.ROC.CD.norm$TP.C.at.cutoff, pch=1)
cat("True cut'off marker:", true.ROC.CD.norm$CUTOFF.CD, "\n")

conv.true.ROC.CD.norm <- conv_meas(true.ROC.CD.norm$FP.D.CD, true.ROC.CD.norm$TP.C.CD)
...
for(i in 1:2000){
  skip_to_next <- FALSE  
  tryCatch({  
normal.copula.rho <- mvdc(normalCopula(rho.par,
                                       dim= 2), 
                          c("unif", "unif"), 
                          list(list(min = 0, max = 1), 
                               list(min = 0, max = 1)), 
                          marginsIdentical = T)
sim.u1.u2 <- rMvdc(size, normal.copula.rho)
...
mle.gauss.two.par <- nlm(ll.joint.two.par.sim,
                         c(-1, -1.5, 0, log(1.3), log(0.8), 0, -6, 1, 0, 1.5, 0.5, 1), 
                         obs.marker=marker.sim,
                         surv.times=time.sim,
                         cens.status=cens.status.sim, gradtol=0.0001, iterlim = 250)

p.hat <- pnorm(mle.gauss.two.par$estimate[3])
scale.hat <- exp(mle.gauss.two.par$estimate[4])
shape.hat <- exp(mle.gauss.two.par$estimate[5])
...
# mle.gauss.two.par$code

a.hat <- exp(mle.gauss.two.par$estimate[1])
b.hat <- exp(mle.gauss.two.par$estimate[2])+1
tau.twopar.hat <- 1- 2/(b.hat*(2+a.hat))
cat("tau sim=", tau.sim, "\n")
cat("tau hat=", tau.twopar.hat, "\n")


est.ROC.CD.two.par <- ROC.CD.two.par(parametros=mle.gauss.two.par$estimate,
                           seq(from=-16, to=11, length=500), t.10)            
points(est.ROC.CD.two.par$FP.D.at.cutoff, est.ROC.CD.two.par$TP.C.at.cutoff)
lines(true.ROC.CD.norm$FP.D.CD, true.ROC.CD.norm$TP.C.CD, col=5)
points(true.ROC.CD.norm$FP.D.at.cutoff, true.ROC.CD.norm$TP.C.at.cutoff, pch=1, col=5)
...
beyene.sim <- tdROC(X=marker.sim, Y=time.sim, 
                    delta=cens.status.sim, tau=t.10)
...
pAUC.Heagherty <- AUC(Heagherty.sim$FP, Heagherty.sim$TP, from=0.2, to=0.4)
...
#Sys.sleep(5)
################
true.ROC.ID.norm <- ROC.ID.norm(parametros=c(atanh(rho.par),-0.5, 
                                             0, log(1.5),
                                             0,-7,0.7,0.5,1,1,1.5),
                                marker=seq(from=-16, to=11, length=500), time=t.10)
plot(true.ROC.ID.norm$FP.D.ID, true.ROC.ID.norm$TP.I.ID, type="l")
points(true.ROC.ID.norm$FP.D.at.cutoff, true.ROC.ID.norm$TP.I.at.cutoff)
# true.ROC.ID.norm$pauc.ID

true.conv.ID.norm <- conv_meas(true.ROC.ID.norm$FP.D.ID, true.ROC.ID.norm$TP.I.ID)

est.ROC.ID.two.par <- ROC.ID.two.par(parametros=mle.gauss.two.par$estimate,
                                     seq(from=-16, to=11, length=500), t.10)
...
pAUC.riskset <- AUC(risksetROC.sim.ID$FP, risksetROC.sim.ID$TP, from=0.2, to=0.4)
...
##############
write(mle.gauss.two.par$code, file="code.txt", append = T)
####
